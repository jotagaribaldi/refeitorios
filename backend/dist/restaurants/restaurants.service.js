...
let RestaurantsService = class RestaurantsService {
    ...
    async findOne(id, tenantId) {
        ...
        if (!r)
            throw new common_1.NotFoundException('Refeitório não encontrado');
        return r;
    }
    ...
    async regenerateQr(id, tenantId) {
        const r = await this.findOne(id, tenantId);
        r.qrCodeToken = (0, uuid_1.v4)();
        await this.repo.save(r);
        const qrDataUrl = await QRCode.toDataURL(JSON.stringify({ restaurantId: r.id, token: r.qrCodeToken }));
